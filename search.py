...
class NPuzzle(Problem):
    ...
    def actions(self,state):
        ...
        if index_blank_square % self.width == 0:
            possible_actions.remove('LEFT')
        if index_blank_square < self.width:
            possible_actions.remove('UP')
        if index_blank_square % self.width == self.width-1:
            possible_actions.remove('RIGHT')
        if index_blank_square > self.N-self.width:
    ...
    def result(self,state,action):
        ...
        delta = {'UP': -self.width, 'DOWN': self.width, 'LEFT': -1, 'RIGHT': 1}
